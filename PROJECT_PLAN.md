# Kế Hoạch Phát Triển Nền Tảng Bán Tool (Software/Tool Sales Platform)

Tài liệu này mô tả chi tiết kiến trúc, tính năng, và lộ trình phát triển cho nền tảng bán công cụ/phần mềm tự động (License Key & File Download).

---

## 1. Mục Tiêu Dự Án
Xây dựng một hệ thống thương mại điện tử trọn gói, tự động hóa 100% quy trình từ lúc khách hàng chọn mua phần mềm đến lúc nhận License Key và Link tải file mềm, không cần sự can thiệp thủ công của Admin.

## 2. Các Tính Năng Cốt Lõi (Core Features)

### 2.1. Dành cho Khách hàng (End-user)
- **Giao diện mua sắm:** Danh mục sản phẩm dạng Grid, phân trang, bộ lọc (Automation, Social, Data Extraction).
...
- **Nhận phần mềm:** Tự động gửi thông tin (License Key và Link Download) ngay sau khi thanh toán thành công lên màn hình và qua Email.
- **Tính năng tăng độ tin cậy (Trust):** Popup live "Khách hàng vừa mua", hiển thị Badge bảo hành.

### 2.2. Dành cho Quản trị viên (Admin)
- **Quản lý Sản phẩm:** Thêm/sửa/xóa phần mềm, định giá, gán category, đính kèm link download.
- **Quản lý License (Inventory):** Nhập danh sách License Key (nếu tạo sẵn) hoặc cấu hình gọi API sinh Key tự động theo thời hạn. Hệ thống tự động phân phối khi có người mua.
...
- **Công nghệ đề xuất:** Node.js (Express/NestJS) hoặc Python (FastAPI/Django) hoặc Java (Spring Boot).
- **Nhiệm vụ:**
  ...
  - Quản lý thuật toán cấp phát License Key và mã hóa Link download chống leech.

### 3.3. Database (Cơ sở dữ liệu)
- **Công nghệ:** PostgreSQL hoặc MySQL.
- **Các bảng chính (Tables Draft):**
  - `products`: id, name, description, price, category, status, download_url.
  - `license_keys`: id, product_id, license_key, is_sold, duration_days.
...
5. **Delivery:** Đổi trạng thái `Order` -> *Success*. Backend xuất 1 License Key (hoặc sinh mới) + Link tải phần mềm và trả về cho Frontend hiển thị.
...
- [ ] Làm chức năng quản lý License và cấp phát Key.

### Phase 3: Tích hợp Thanh Toán tự động
- [ ] Viết API Webhook nhận biến động số dư.
- [ ] Viết luồng xử lý và tự động xuất License Key.
